...
(() => {
  ...
  var dataFormatSchema = esm_default.object({
    atb: esm_default.string().optional(),
    totalCount: esm_default.number(),
    totalPeriod: esm_default.enum(["install-time"]),
    trackerCompaniesPeriod: esm_default.enum(["last-hour", "last-day"]),
    trackerCompanies: esm_default.array(
      esm_default.object({
        displayName: esm_default.string(),
        count: esm_default.number()
      })
    )
  });
  var disconnectMessage = esm_default.object({
    messageType: esm_default.literal(events.outgoing.newTabPage_disconnect)
  });
  var dataMessage = esm_default.object({
    messageType: esm_default.literal(events.outgoing.newTabPage_data),
    options: dataFormatSchema
  });
  var outgoing = esm_default.discriminatedUnion("messageType", [dataMessage, disconnectMessage]);
  var heartbeatMessage = esm_default.object({
    messageType: esm_default.literal(events.incoming.newTabPage_heartbeat)
  });
  var incoming = esm_default.discriminatedUnion("messageType", [heartbeatMessage]);
